RSpec.describe BatchHelper, type: :helper do
  ...
  describe "#batch_list_table" do
    ...
    it "returns a table with batch details" do
      ...
      expect(subject).to have_selector("td", text: "Submitted")
      expect(subject).to have_selector("td", text: "Not yet submitted")
      expect(subject).to have_selector("td", text: "Draft")
    end
  end
  ...
  describe "#batch_example_action" do
    it do
      ...
      expect(batch_example_action).to have_selector("table tr", count: 3)
  ...
  context "with a single batch" do
    ...
    describe "#batch_status_tag" do
      it do
        expect(batch_status_tag(batch)).to have_selector("strong", class: "govuk-tag--green", text: "Submitted")
      end
    end
    ...
    describe "#admin_batch_list_table" do
      ...
      let(:appropriate_body_period) { FactoryBot.create(:appropriate_body_period, name: "The Appropriate Body") }
      ...
      it do
        ...
        expect(table).to have_selector("td", text: "Submitted")
        expect(table).to have_selector("td", text: "Claim")
        expect(table).to have_selector("td", text: "Draft")
      end
    end
  end
end
