      ...
      context "when the email is valid" do
        ...
        it "updates the email only after confirmation" do
          ...
          expect { post path_for_step("check-answers") }
            .to change { ect_at_school_period.reload.email }
            .to(new_email)
        ...
        it "creates an event only after confirmation" do
          allow(Events::Record).to receive(:record_teacher_email_changed_event!)

          post(path_for_step("edit"), params:)

          expect(Events::Record).not_to have_received(:record_teacher_email_changed_event!)
          expect(response).to redirect_to(path_for_step("check-answers"))
          ...
          expect(Events::Record).to have_received(:record_teacher_email_changed_event!)
          expect(response).to redirect_to(path_for_step("confirmation"))
        end
      end
...
private
  ...
  def path_for_step(step)
    "/school/ects/#{ect_at_school_period.id}/change-email-address/#{step}"
  end
end
