  ...
  describe "GET /admin/organisations/appropriate-bodies/current-ects" do
    ...
    context "with an authenticated DfE user" do
      ...
      it "only shows ECTs who belong to this appropriate body" do
        expect(Teachers::Search).to receive(:new).with(appropriate_bodies: appropriate_body_period).once.and_call_original
        expect(Teachers::Search).to receive(:new).with(query_string: nil, appropriate_bodies: appropriate_body_period).once.and_call_original
      ...
      context "when searching by name" do
        ...
        it "only shows matching ECTs" do
          get "/admin/organisations/appropriate-bodies/#{appropriate_body_period.id}/current-ects?q=#{search_term}"
        ...
        it "only shows ECTs who belong to this appropriate body" do
          expect(Teachers::Search).to receive(:new).with(appropriate_bodies: appropriate_body_period).once.and_call_original
          expect(Teachers::Search).to receive(:new).with(query_string: search_term, appropriate_bodies: appropriate_body_period).once.and_call_original
