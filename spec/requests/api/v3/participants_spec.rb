RSpec.describe "Participants API", type: :request do
  ...
  let!(:ect_teacher) { FactoryBot.create(:teacher, api_ect_training_record_created_at: Time.zone.now, api_mentor_training_record_created_at: Time.zone.now) }
  let!(:mentor_teacher) { FactoryBot.create(:teacher, api_ect_training_record_created_at: Time.zone.now, api_mentor_training_record_created_at: Time.zone.now) }
  let!(:both_teacher) { FactoryBot.create(:teacher, api_ect_training_record_created_at: Time.zone.now, api_mentor_training_record_created_at: Time.zone.now) }
      ...
      expect(attrs1).to include(
        "ecf_enrolments" => contain_exactly(
          include(
            ...
            "created_at" => ect_teacher.api_ect_training_record_created_at&.rfc3339
          )
        )
      )
      ...
      expect(attrs2).to include(
        "ecf_enrolments" => contain_exactly(
          include(
            ...
            "lead_provider" => lead_provider.name,
            "created_at" => mentor_teacher.api_mentor_training_record_created_at&.rfc3339
          )
        )
      )
      ...
      expect(attrs3).to include(
        "ecf_enrolments" => contain_exactly(
          include(
            ...
            "lead_provider" => lead_provider.name,
            "created_at" => both_teacher.api_ect_training_record_created_at&.rfc3339
          ),
          include(
            "training_record_id" => both_teacher.api_mentor_training_record_id,
            "email" => both_mentor_period.email,
            "lead_provider" => lead_provider.name,
            "created_at" => both_teacher.api_mentor_training_record_created_at&.rfc3339
          )
        )
      )
    end
