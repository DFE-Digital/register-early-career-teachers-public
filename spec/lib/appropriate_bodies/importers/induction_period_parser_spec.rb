RSpec.describe AppropriateBodies::Importers::InductionPeriodParser do
  subject(:parser) do
    described_class.new(
      data_csv: sample_csv_data,
      logger: fake_logger
    )
  end

  let(:row_class) { described_class::Row }

  let(:fake_logger) { double(Logger, error: true) }

  let!(:ab_1) { FactoryBot.create(:appropriate_body_period, :local_authority) }
  let!(:ab_2) { FactoryBot.create(:appropriate_body_period, :local_authority) }
  let!(:ab_3) { FactoryBot.create(:appropriate_body_period, :local_authority) }
  let!(:ab_4) { FactoryBot.create(:appropriate_body_period, :local_authority) }
  let!(:ab_5) { FactoryBot.create(:appropriate_body_period, :local_authority) }

  let!(:ect_1) { FactoryBot.create(:teacher) }
  let!(:ect_2) { FactoryBot.create(:teacher) }
  let!(:ect_3) { FactoryBot.create(:teacher) }
  let!(:ect_4) { FactoryBot.create(:teacher, id: described_class::UNWANTED_TEACHER_IDS[0]) }
  let!(:ect_5) { FactoryBot.create(:teacher, id: described_class::UNWANTED_TEACHER_IDS[1]) }

  # Date.new(2021, 9, 1) ECF cuttoff
  # 1. Before ECF cutoff
  #
  let(:sample_csv_data) do
    <<~CSV
      appropriate_body_id,started_on,finished_on,induction_programme_choice,number_of_terms,trn
      #{ab_1.dqt_id},01/01/2012 00:00:00,10/31/2012 00:00:00,Core Induction Programme,3,#{ect_1.trn}
      #{ab_2.dqt_id},12/12/2021 00:00:00,10/31/2022 00:00:00,Core Induction Programme,3,#{ect_1.trn}
      #{ab_3.dqt_id},09/02/2022 00:00:00,11/13/2023 00:00:00,School-based Induction Programme,3,#{ect_2.trn}
      #{ab_4.dqt_id},10/31/2022 00:00:00,02/01/2023 00:00:00,Full Induction Programme,3,#{ect_3.trn}
      #{ab_5.dqt_id},10/31/2023 00:00:00,02/01/2024 00:00:00,,3,#{ect_3.trn}
    CSV
  end

  describe "#rows" do
    it "converts CSV::Rows to InductionPeriodParser::Rows" do
      expect(parser.rows).to all(be_a(row_class))
    end

    it "converts all rows" do
      expect(parser.rows.count).to be(5)
    end

    describe "mapping induction programmes" do
      it "converts names to enum values" do
        mappings = {
          parser.rows.find { |r| r.legacy_appropriate_body_id == ab_1.dqt_id } => "pre_september_2021",
          parser.rows.find { |r| r.legacy_appropriate_body_id == ab_2.dqt_id } => "cip",
          parser.rows.find { |r| r.legacy_appropriate_body_id == ab_3.dqt_id } => "diy",
          parser.rows.find { |r| r.legacy_appropriate_body_id == ab_4.dqt_id } => "fip",
          parser.rows.find { |r| r.legacy_appropriate_body_id == ab_5.dqt_id } => "unknown"
        }
        mappings.each do |row, expected_induction_programme|
          induction_programme = row.to_hash.fetch(:induction_programme)
          expect(induction_programme).to eql(expected_induction_programme)
        end
      end
    end
  end

  describe "#periods_by_trn" do
    it "groups all trns" do
      expect(parser.periods_by_trn.count).to be(3)
    end

    it "orders by trn" do
      expect(parser.periods_by_trn.keys).to eq([ect_2.trn, ect_3.trn, ect_1.trn])
    end

    context "when all rows are valid" do
      it "keeps all rows" do
        periods = parser.periods_by_trn.values.flatten
        expect(periods.count).to eq(parser.rows.count)
      end
    end

    context "when some rows are invalid" do
      # 1. off-shore AB
      # 2. missing end date
      # 3. teacher record missing
      let(:sample_csv_data) do
        <<~CSV
          appropriate_body_id,started_on,finished_on,induction_programme_choice,number_of_terms,trn
          #{ab_1.dqt_id},01/01/2012 00:00:00,10/31/2012 00:00:00,Core Induction Programme,3,#{ect_1.trn}
          #{ab_2.dqt_id},12/12/2021 00:00:00,10/31/2022 00:00:00,Core Induction Programme,3,#{ect_1.trn}
          #{ab_3.dqt_id},09/02/2022 00:00:00,11/13/2023 00:00:00,School-based Induction Programme,3,#{ect_2.trn}
          #{ab_4.dqt_id},10/31/2022 00:00:00,02/01/2023 00:00:00,Full Induction Programme,3,#{ect_3.trn}
          #{ab_5.dqt_id},10/31/2023 00:00:00,02/01/2024 00:00:00,,3,#{ect_3.trn}
        CSV
      end

      it "rejects some rows" do
        periods = parser.periods_by_trn.values.flatten
        expect(periods.count).to be < parser.rows.count
      end
    end
  end

  # FIXME: why #periods_as_hashes_by_trn?
  describe "#periods_as_hashes_by_trn" do
    # FIXME: doesn't zero number of terms when the end date is missing
    describe "number of terms" do
      context "when number_of_terms is present and finished_on is blank" do
        let(:sample_csv_data) do
          <<~CSV
            appropriate_body_id,started_on,finished_on,induction_programme_choice,number_of_terms,trn
            #{ab_1.dqt_id},01/01/2012 00:00:00,,,3,#{ect_1.trn}
          CSV
        end

        it "sets the number_of_terms to nil" do
          row_data = parser.periods_as_hashes_by_trn[ect_1.trn].first
          expect(row_data.fetch(:started_on)).to eql(Date.new(2012, 1, 1))
          expect(row_data.fetch(:number_of_terms)).to be_nil
        end
      end
    end

    describe "finished_on" do
      context "when started_on and finished_on are the same day" do
        let(:sample_csv_data) do
          <<~CSV
            appropriate_body_id,started_on,finished_on,induction_programme_choice,number_of_terms,trn
            #{ab_1.dqt_id},01/01/2012 00:00:00,01/01/2012 00:00:00,,3,#{ect_1.trn}
            #{ab_1.dqt_id},01/01/2021 00:00:00,,,3,#{ect_1.trn}
          CSV
        end

        it "bumps the finish date by one day" do
          row_data = parser.periods_as_hashes_by_trn[ect_1.trn].first
          expect(row_data.fetch(:started_on)).to eql(Date.new(2012, 1, 1))
          expect(row_data.fetch(:finished_on)).to eql(Date.new(2012, 1, 2))
        end
      end
    end

    describe "cutoff dates for old appropriate bodies" do
      context "when the appropriate body ID appears in the cutoff list" do
        # teacher on row 3 hasn't been made yet
        # FIXME: 1. replace TRNs with ones from teachers made above, 2. do "#{ect_1.trn}"
        let(:sample_csv_data) do
          <<~CSV
            appropriate_body_id,started_on,finished_on,induction_programme_choice,number_of_terms,trn
            #{ab_1.dqt_id},01/01/2019 00:00:00,10/31/2024 00:00:00,Core Induction Programme,3,#{ect_1.trn}
            #{ab_1.dqt_id},12/12/2020 00:00:00,,Core Induction Programme,3,2600072
            #{ab_1.dqt_id},09/05/2024 00:00:00,,Core Induction Programme,3,2600073
            #{ab_2.dqt_id},01/01/2019 00:00:00,10/31/2024 00:00:00,Core Induction Programme,3,2600074
          CSV
        end

        context "when the finish date is later than the cutoff date" do
          # 1.day.before(described_class::ECF_CUTOFF)

          it "cuts off the induction period that appears in the cutoff list on 2024-08-31" do
            row_data = parser.periods_as_hashes_by_trn[ect_1.trn].first
            expect(row_data.fetch(:finished_on)).to eql(Date.new(2024, 8, 31))
          end
        end

        context "when the finish date is null" do
          # FIXME: ect_2 etc
          it "cuts off the induction period that appears in the cutoff list on 2024-08-31" do
            row_data = parser.periods_as_hashes_by_trn["2600072"].first
            expect(row_data.fetch(:finished_on)).to eql(Date.new(2024, 8, 31))
          end
        end

        context "when the start date is later than the cutoff date" do
          it "sets the finish date to one day later than the start date" do
            row_data = parser.periods_as_hashes_by_trn["2600073"].first
            expect(row_data.fetch(:started_on)).to eql(Date.new(2024, 9, 5))
            expect(row_data.fetch(:finished_on)).to eql(Date.new(2024, 9, 6))
          end
        end

        context "when the id is not in the cutoff list" do
          it "leaves the dates untouched" do
            row_data = parser.periods_as_hashes_by_trn["2600074"].first
            expect(row_data.fetch(:started_on)).to eql(Date.new(2019, 1, 1))
            expect(row_data.fetch(:finished_on)).to eql(Date.new(2024, 10, 31))
          end
        end
      end
    end

    describe "invalid periods" do
      describe "started_on is nil" do
        let(:sample_csv_data) do
          <<~CSV
            appropriate_body_id,started_on,finished_on,induction_programme_choice,number_of_terms,trn
            #{ab_1.dqt_id},,,,3,#{ect_1.trn}
          CSV
        end

        it "does not import the row" do
          expect(parser.periods_as_hashes_by_trn).to be_empty
        end

        it "logs an error" do
          parser.periods_as_hashes_by_trn
          expect(fake_logger).to have_received(:error).once.with(/started_on is nil/)
        end
      end

      describe "started_on is invalid" do
        let(:sample_csv_data) do
          <<~CSV
            appropriate_body_id,started_on,finished_on,induction_programme_choice,number_of_terms,trn
            #{ab_1.dqt_id},01/01/0001 00:00:00,,,3,#{ect_1.trn}
          CSV
        end

        it "does not import the row" do
          expect(parser.periods_as_hashes_by_trn).to be_empty
        end

        it "logs an error" do
          parser.periods_as_hashes_by_trn
          expect(fake_logger).to have_received(:error).once.with(/started_on is 0001-01-01/)
        end
      end

      describe "started_on is greater than finished_on" do
        let(:sample_csv_data) do
          <<~CSV
            appropriate_body_id,started_on,finished_on,induction_programme_choice,number_of_terms,trn
            #{ab_1.dqt_id},02/02/2023 00:00:00,01/01/2023 00:00:00,,3,#{ect_1.trn}
          CSV
        end

        it "does not import the row" do
          expect(parser.periods_as_hashes_by_trn).to be_empty
        end

        it "logs an error" do
          parser.periods_as_hashes_by_trn
          expect(fake_logger).to have_received(:error).once.with(/started_on is greater than finished_on/)
        end
      end
    end

    describe "rebuilding periods" do
      context "when an ECT has one induction period with one AB" do
        let(:sample_csv_data) do
          <<~CSV
            appropriate_body_id,started_on,finished_on,induction_programme_choice,number_of_terms,trn
            #{ab_1.dqt_id},01/01/2012 00:00:00,,,3,#{ect_1.trn}
          CSV
        end

        # received a finished date and terms
        #
        # expected [{appropriate_body_period_id: nil, started_on: Sun, 01 Jan 2012, finished_on: nil, induction_programme: "pre_september_2021", number_of_terms: nil, outcome: nil}]}
        # received [{appropriate_body_period_id: nil, started_on: Sun, 01 Jan 2012, finished_on: Sat, 31 Aug 2024, induction_programme: "pre_september_2021", number_of_terms: 3, outcome: nil}]
        it "contains the original row untouched" do
          expect(parser.periods_as_hashes_by_trn).to eql(
            {
              ect_1.trn => [
                row_class.new(
                  started_on: Date.new(2012, 1, 1),
                  finished_on: nil,
                  induction_programme: nil,
                  legacy_appropriate_body_id: ab_1.dqt_id,
                  trn: ect_1.trn,
                  number_of_terms: 3,
                  notes: []
                ).to_hash
              ]
            }
          )
        end
      end

      context "when an ECT has two induction periods that have the same programme type with one AB" do
        context "and the first period ends after the second period has started" do
          let(:sample_csv_data) do
            <<~CSV
              appropriate_body_id,started_on,finished_on,induction_programme_choice,number_of_terms,trn
              #{ab_1.dqt_id},01/01/2012 00:00:00,03/03/2012 00:00:00,Full Induction Programme,3,#{ect_1.trn}
              #{ab_1.dqt_id},02/02/2012 00:00:00,04/04/2012 00:00:00,Full Induction Programme,3,#{ect_1.trn}
              #{ab_1.dqt_id},02/02/2021 00:00:00,,Full Induction Programme,,#{ect_1.trn}
            CSV
          end

          it "combines the two periods so the new period has the earliest start date and the latest finish date" do
            expect(parser.periods_as_hashes_by_trn).to eql(
              {
                ect_1.trn => [
                  row_class.new(
                    started_on: Date.new(2012, 1, 1),
                    finished_on: Date.new(2012, 4, 4),
                    induction_programme: "Full Induction Programme",
                    legacy_appropriate_body_id: ab_1.dqt_id,
                    trn: ect_1.trn,
                    number_of_terms: 3,
                    notes: []
                  ).to_hash,

                  # unrelated ongoing record so induction periods are included by the import
                  row_class.new(
                    started_on: Date.new(2021, 2, 2),
                    finished_on: nil,
                    induction_programme: "Full Induction Programme",
                    legacy_appropriate_body_id: ab_1.dqt_id,
                    trn: ect_1.trn,
                    number_of_terms: 0,
                    notes: []
                  ).to_hash
                ]
              }
            )
          end
        end

        context "and the first period has an end date and terms but the second does not" do
          let(:sample_csv_data) do
            <<~CSV
              appropriate_body_id,started_on,finished_on,induction_programme_choice,number_of_terms,trn
              #{ab_1.dqt_id},01/01/2012 00:00:00,03/03/2012 00:00:00,Full Induction Programme,3,#{ect_1.trn}
              #{ab_1.dqt_id},01/01/2012 00:00:00,,Full Induction Programme,,#{ect_1.trn}
            CSV
          end

          it "keeps the finished (first) period" do
            expect(parser.periods_as_hashes_by_trn).to eql(
              {
                ect_1.trn => [
                  row_class.new(
                    started_on: Date.new(2012, 1, 1),
                    finished_on: Date.new(2012, 3, 3),
                    induction_programme: "Full Induction Programme",
                    legacy_appropriate_body_id: ab_1.dqt_id,
                    trn: ect_1.trn,
                    number_of_terms: 3,
                    notes: []
                  ).to_hash,
                ]
              }
            )
          end
        end

        context "and the second period has an end date and terms but the first does not" do
          let(:sample_csv_data) do
            <<~CSV
              appropriate_body_id,started_on,finished_on,induction_programme_choice,number_of_terms,trn
              #{ab_1.dqt_id},01/01/2012 00:00:00,,Full Induction Programme,,#{ect_1.trn}
              #{ab_1.dqt_id},01/01/2012 00:00:00,03/03/2012 00:00:00,Full Induction Programme,3,#{ect_1.trn}
            CSV
          end

          it "keeps the finished (second) period" do
            expect(parser.periods_as_hashes_by_trn).to eql(
              {
                ect_1.trn => [
                  row_class.new(
                    started_on: Date.new(2012, 1, 1),
                    finished_on: Date.new(2012, 3, 3),
                    induction_programme: "Full Induction Programme",
                    legacy_appropriate_body_id: ab_1.dqt_id,
                    trn: ect_1.trn,
                    number_of_terms: 3,
                    notes: []
                  ).to_hash,
                ]
              }
            )
          end
        end

        context "and the second period ends after the first period has started" do
          let(:sample_csv_data) do
            <<~CSV
              appropriate_body_id,started_on,finished_on,induction_programme_choice,number_of_terms,trn
              #{ab_1.dqt_id},02/02/2012 00:00:00,04/04/2012 00:00:00,Full Induction Programme,3,#{ect_1.trn}
              #{ab_1.dqt_id},01/01/2012 00:00:00,03/03/2012 00:00:00,Full Induction Programme,4,#{ect_1.trn}
              #{ab_1.dqt_id},01/01/2021 00:00:00,,Full Induction Programme,,#{ect_1.trn}
            CSV
          end

          it "combines the two periods so the new period has the earliest start date and the latest finish date" do
            expect(parser.periods_as_hashes_by_trn).to eql(
              {
                ect_1.trn => [
                  row_class.new(
                    started_on: Date.new(2012, 1, 1),
                    finished_on: Date.new(2012, 4, 4),
                    induction_programme: "Full Induction Programme",
                    legacy_appropriate_body_id: ab_1.dqt_id,
                    trn: ect_1.trn,
                    number_of_terms: 4
                  ).to_hash,

                  # unrelated ongoing record so induction periods are included by the import
                  row_class.new(
                    started_on: Date.new(2021, 1, 1),
                    finished_on: nil,
                    induction_programme: "Full Induction Programme",
                    legacy_appropriate_body_id: ab_1.dqt_id,
                    trn: ect_1.trn,
                    number_of_terms: 0
                  ).to_hash
                ]
              }
            )
          end

          it "keeps the higher number of terms" do
            number_of_terms = parser.periods_as_hashes_by_trn.dig(ect_1.trn, 0, :number_of_terms)

            expect(number_of_terms).to be(4)
          end
        end

        context "and the first period contains the second" do
          let(:sample_csv_data) do
            <<~CSV
              appropriate_body_id,started_on,finished_on,induction_programme_choice,number_of_terms,trn
              #{ab_1.dqt_id},01/01/2012 00:00:00,04/04/2012 00:00:00,Full Induction Programme,2,#{ect_1.trn}
              #{ab_1.dqt_id},02/02/2012 00:00:00,03/03/2012 00:00:00,Full Induction Programme,3,#{ect_1.trn}
              #{ab_1.dqt_id},02/02/2021 00:00:00,,Full Induction Programme,3,#{ect_1.trn}
            CSV
          end

          it "keeps the dates from the first" do
            expect(parser.periods_as_hashes_by_trn).to eql(
              {
                ect_1.trn => [
                  row_class.new(
                    started_on: Date.new(2012, 1, 1),
                    finished_on: Date.new(2012, 4, 4),
                    induction_programme: "Full Induction Programme",
                    legacy_appropriate_body_id: ab_1.dqt_id,
                    trn: ect_1.trn,
                    number_of_terms: 3,
                    notes: []
                  ).to_hash,

                  # unrelated ongoing record so induction periods are included by the import
                  row_class.new(
                    started_on: Date.new(2021, 2, 2),
                    finished_on: nil,
                    induction_programme: "Full Induction Programme",
                    legacy_appropriate_body_id: ab_1.dqt_id,
                    trn: ect_1.trn,
                    number_of_terms: 3,
                    notes: []
                  ).to_hash
                ]
              }
            )
          end
        end

        context "and the second period contains the first" do
          let(:sample_csv_data) do
            <<~CSV
              appropriate_body_id,started_on,finished_on,induction_programme_choice,number_of_terms,trn
              #{ab_1.dqt_id},02/02/2012 00:00:00,03/03/2012 00:00:00,Full Induction Programme,1,#{ect_1.trn}
              #{ab_1.dqt_id},01/01/2012 00:00:00,04/04/2012 00:00:00,Full Induction Programme,4,#{ect_1.trn}
              #{ab_1.dqt_id},01/01/2021 00:00:00,,Full induction programme,0,#{ect_1.trn}
            CSV
          end

          it "only the second is kept" do
            expect(parser.periods_as_hashes_by_trn).to eql(
              {
                ect_1.trn => [
                  row_class.new(
                    started_on: Date.new(2012, 1, 1),
                    finished_on: Date.new(2012, 4, 4),
                    induction_programme: "Full Induction Programme",
                    legacy_appropriate_body_id: ab_1.dqt_id,
                    trn: ect_1.trn,
                    number_of_terms: 4,
                    notes: []
                  ).to_hash,

                  # unrelated ongoing record so induction periods are included by the import
                  row_class.new(
                    started_on: Date.new(2021, 1, 1),
                    finished_on: nil,
                    induction_programme: "Full Induction Programme",
                    legacy_appropriate_body_id: ab_1.dqt_id,
                    trn: ect_1.trn,
                    number_of_terms: 0,
                    notes: []
                  ).to_hash
                ]
              }
            )
          end
        end
      end

      context "when an ECT has two induction periods that have different programme types with one AB" do
        context "when the periods do not overlap" do
          let(:sample_csv_data) do
            <<~CSV
              appropriate_body_id,started_on,finished_on,induction_programme_choice,number_of_terms,trn
              #{ab_2.dqt_id},01/01/2012 00:00:00,03/03/2012 00:00:00,Full Induction Programme,4,#{ect_1.trn}
              #{ab_2.dqt_id},03/03/2012 00:00:00,05/05/2012 00:00:00,Core Induction Programme,2,#{ect_1.trn}
              #{ab_2.dqt_id},03/03/2021 00:00:00,,Full Induction Programme,2,#{ect_1.trn}
            CSV
          end

          it "both are kept" do
            expect(parser.periods_as_hashes_by_trn).to eql(
              {
                ect_1.trn => [
                  row_class.new(
                    started_on: Date.new(2012, 1, 1),
                    finished_on: Date.new(2012, 3, 3),
                    induction_programme: "Full Induction Programme",
                    legacy_appropriate_body_id: ab_2.dqt_id,
                    trn: ect_1.trn,
                    number_of_terms: 4,
                    notes: []
                  ).to_hash,

                  row_class.new(
                    started_on: Date.new(2012, 3, 3),
                    finished_on: Date.new(2012, 5, 5),
                    induction_programme: "Core Induction Programme",
                    legacy_appropriate_body_id: ab_2.dqt_id,
                    trn: ect_1.trn,
                    number_of_terms: 2,
                    notes: []
                  ).to_hash,

                  # unrelated ongoing record so induction periods are included by the import
                  row_class.new(
                    started_on: Date.new(2021, 3, 3),
                    finished_on: nil,
                    induction_programme: "Full Induction Programme",
                    legacy_appropriate_body_id: ab_2.dqt_id,
                    trn: ect_1.trn,
                    number_of_terms: 2,
                    notes: []
                  ).to_hash
                ]
              }
            )
          end
        end

        context "when the periods overlap" do
          let(:sample_csv_data) do
            <<~CSV
              appropriate_body_id,started_on,finished_on,induction_programme_choice,number_of_terms,trn
              #{ab_2.dqt_id},01/01/2012 00:00:00,05/05/2012 00:00:00,Full Induction Programme,4,#{ect_1.trn}
              #{ab_2.dqt_id},04/04/2012 00:00:00,06/06/2012 00:00:00,Core Induction Programme,2,#{ect_1.trn}
              #{ab_2.dqt_id},03/03/2021 00:00:00,,Full Induction Programme,2,#{ect_1.trn}
            CSV
          end

          it "the earlier one is curtailed so it does not clash with the later one" do
            expect(parser.periods_as_hashes_by_trn).to eql(
              {
                ect_1.trn => [
                  row_class.new(
                    started_on: Date.new(2012, 1, 1),
                    finished_on: Date.new(2012, 4, 4),
                    induction_programme: "Full Induction Programme",
                    legacy_appropriate_body_id: ab_2.dqt_id,
                    trn: ect_1.trn,
                    number_of_terms: 4,
                    notes: []
                  ).to_hash,

                  row_class.new(
                    started_on: Date.new(2012, 4, 4),
                    finished_on: Date.new(2012, 6, 6),
                    induction_programme: "Core Induction Programme",
                    legacy_appropriate_body_id: ab_2.dqt_id,
                    trn: ect_1.trn,
                    number_of_terms: 2,
                    notes: []
                  ).to_hash,

                  # unrelated ongoing record so induction periods are included by the import
                  row_class.new(
                    started_on: Date.new(2021, 3, 3),
                    finished_on: nil,
                    induction_programme: "Full Induction Programme",
                    legacy_appropriate_body_id: ab_2.dqt_id,
                    trn: ect_1.trn,
                    number_of_terms: 2,
                    notes: []
                  ).to_hash
                ]
              }
            )
          end
        end
      end

      context "when an ECT has two induction period with different ABs" do
        context "when one contains the other" do
          let(:sample_csv_data) do
            <<~CSV
              appropriate_body_id,started_on,finished_on,induction_programme_choice,number_of_terms,trn
              #{ab_2.dqt_id},02/02/2012 00:00:00,03/03/2012 00:00:00,Full Induction Programme,4,#{ect_1.trn}
              #{ab_1.dqt_id},01/01/2012 00:00:00,05/05/2012 00:00:00,Full Induction Programme,2,#{ect_1.trn}
              #{ab_1.dqt_id},03/03/2021 00:00:00,,Full Induction Programme,2,#{ect_1.trn}
            CSV
          end

          it "both are discarded" do
            expect(parser.periods_as_hashes_by_trn.values.length).to be(1)
          end

          it "logs the error message" do
            parser.periods_as_hashes_by_trn
            expect(fake_logger).to have_received(:error).once.with(/two induction periods with different appropriate bodies where one contains the other/)
          end

          it "logs the affected IDs" do
            parser.periods_as_hashes_by_trn
            # FIXME: replace hardcoded trn and uuid
            expect(fake_logger).to have_received(:error).once.with(/trn: 3600071 appropriate_body_period_id: \["1ddf3e82-c1ae-e311-b8ed-005056822391", "025e61e7-ec32-eb11-a813-000d3a228dfc"\]/)
          end
        end

        context "when both start on the same day" do
          let(:sample_csv_data) do
            <<~CSV
              appropriate_body_id,started_on,finished_on,induction_programme_choice,number_of_terms,trn
              #{ab_2.dqt_id},02/02/2012 00:00:00,03/03/2012 00:00:00,Full Induction Programme,4,#{ect_1.trn}
              #{ab_1.dqt_id},02/02/2012 00:00:00,,Full Induction Programme,2,#{ect_1.trn}
              #{ab_1.dqt_id},03/03/2021 00:00:00,,Full Induction Programme,2,#{ect_1.trn}
            CSV
          end

          it "both are discarded" do
            expect(parser.periods_as_hashes_by_trn.values.length).to be(1)
          end

          it "logs the error message" do
            parser.periods_as_hashes_by_trn.values.length
            expect(fake_logger).to have_received(:error).once.with(/two induction periods with different appropriate bodies that start on the same day found/)
          end

          it "logs the affected IDs" do
            # FIXME: replace hardcoded trn and uuid
            parser.periods_as_hashes_by_trn.values.length
            expect(fake_logger).to have_received(:error).once.with(/trn: 3600071 appropriate_body_period_id: \["025e61e7-ec32-eb11-a813-000d3a228dfc", "1ddf3e82-c1ae-e311-b8ed-005056822391"\]/)
          end
        end

        context "when the periods do not overlap" do
          let(:sample_csv_data) do
            <<~CSV
              appropriate_body_id,started_on,finished_on,induction_programme_choice,number_of_terms,trn
              #{ab_2.dqt_id},01/01/2012 00:00:00,03/03/2012 00:00:00,Full Induction Programme,4,#{ect_1.trn}
              #{ab_1.dqt_id},03/03/2012 00:00:00,05/05/2012 00:00:00,Full Induction Programme,2,#{ect_1.trn}
              #{ab_1.dqt_id},03/03/2021 00:00:00,,Full Induction Programme,2,#{ect_1.trn}
            CSV
          end

          it "both are kept" do
            expect(parser.periods_as_hashes_by_trn).to eql(
              {
                ect_1.trn => [
                  row_class.new(
                    started_on: Date.new(2012, 1, 1),
                    finished_on: Date.new(2012, 3, 3),
                    induction_programme: "Full Induction Programme",
                    legacy_appropriate_body_id: ab_2.dqt_id,
                    trn: ect_1.trn,
                    number_of_terms: 4,
                    notes: []
                  ).to_hash,

                  row_class.new(
                    started_on: Date.new(2012, 3, 3),
                    finished_on: Date.new(2012, 5, 5),
                    induction_programme: "Full Induction Programme",
                    legacy_appropriate_body_id: ab_1.dqt_id,
                    trn: ect_1.trn,
                    number_of_terms: 2,
                    notes: []
                  ).to_hash,

                  # unrelated ongoing record so induction periods are included by the import
                  row_class.new(
                    started_on: Date.new(2021, 3, 3),
                    finished_on: nil,
                    induction_programme: "Full Induction Prorgramme",
                    legacy_appropriate_body_id: ab_1.dqt_id,
                    trn: ect_1.trn,
                    number_of_terms: 2,
                    notes: []
                  ).to_hash
                ]
              }
            )
          end
        end

        context "when the periods overlap" do
          let(:sample_csv_data) do
            <<~CSV
              appropriate_body_id,started_on,finished_on,induction_programme_choice,number_of_terms,trn
              #{ab_2.dqt_id},01/01/2012 00:00:00,03/03/2012 00:00:00,Full Induction Programme,1,#{ect_1.trn}
              #{ab_1.dqt_id},02/02/2012 00:00:00,05/05/2012 00:00:00,Full Induction Programme,4,#{ect_1.trn}
              #{ab_1.dqt_id},02/02/2021 00:00:00,,Full Induction Programme,4,#{ect_1.trn}
            CSV
          end

          it "both are kept but the earlier one is shortened to prevent overlap with the second" do
            expect(parser.periods_as_hashes_by_trn).to eql(
              {
                ect_1.trn => [
                  row_class.new(
                    started_on: Date.new(2012, 1, 1),
                    finished_on: Date.new(2012, 2, 2),
                    induction_programme: "Full Induction Programme",
                    legacy_appropriate_body_id: ab_2.dqt_id,
                    trn: ect_1.trn,
                    number_of_terms: 1
                  ).to_hash,

                  row_class.new(
                    started_on: Date.new(2012, 2, 2),
                    finished_on: Date.new(2012, 5, 5),
                    induction_programme: "Full Induction Programme",
                    legacy_appropriate_body_id: ab_1.dqt_id,
                    trn: ect_1.trn,
                    number_of_terms: 4
                  ).to_hash,

                  # unrelated ongoing record so induction periods are included by the import
                  row_class.new(
                    started_on: Date.new(2021, 2, 2),
                    finished_on: nil,
                    induction_programme: "Full Induction Programme",
                    legacy_appropriate_body_id: ab_1.dqt_id,
                    trn: ect_1.trn,
                    number_of_terms: 4
                  ).to_hash
                ]
              }
            )
          end
        end
      end
    end
  end
end
