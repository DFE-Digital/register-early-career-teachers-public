RSpec.describe "appropriate_bodies/teachers/record_released_induction/new.html.erb" do
  ...
  let(:record_release) do
    AppropriateBodies::RecordRelease.new(teacher:, appropriate_body: appropriate_body_period, author:)
  end
  ...
  it "has a date field for the leaving date" do
    expect(rendered).to have_css("legend", text: "When did they move from #{appropriate_body_period.name}?")
