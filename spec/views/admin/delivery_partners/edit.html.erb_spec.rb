RSpec.describe 'admin/delivery_partners/edit.html.erb' do
  ...
  it 'renders a form that submits to the update action' do
    render

    expect(rendered).to have_css("form[action='#{admin_delivery_partner_path(delivery_partner)}'][method='post']")
    expect(rendered).to have_css("input[name='_method'][value='patch']", visible: :hidden)
  end
