RSpec.describe PassECTInductionJob, type: :job do
  ...
  describe '#perform' do
    context "when the API call is successful" do
      before do
        allow(api_client).to receive(:pass_induction!).with(trn:, start_date:, completed_date:)
      end

      it "calls the API client with correct parameters" do
        expect(api_client).to receive(:pass_induction!).with(trn:, start_date:, completed_date:)
