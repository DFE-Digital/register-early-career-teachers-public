RSpec.describe Admin::Teachers::UndoRegistrationWizard::SelectSchoolPeriodStep do
  subject(:step) { described_class.new(wizard:, at_school_period_gid:) }

  let(:store) { FactoryBot.build(:session_repository) }
  let(:at_school_period_gid) { "at-school-period-gid" }
  let(:selected_at_school_period) { instance_double(ECTAtSchoolPeriod) }
  let(:wizard) do
    instance_double(
      Admin::Teachers::UndoRegistrationWizard::Wizard,
      store:,
      teacher_name: "Kakashi Hatake",
      at_school_period_from_gid: selected_at_school_period
    )
  end
  ...
  before do
    ...
    allow(wizard).to receive(:step_params)
      .and_return(ActionController::Parameters.new(at_school_period_gid:).permit(:at_school_period_gid))
  end

  describe "#save!" do
    it "stores the selected school period" do
      expect { step.save! }
        .to change(store, :at_school_period_gid)
        .from(nil)
        .to(at_school_period_gid)
    end

    context "when the school period is invalid" do
      let(:selected_at_school_period) { nil }

      it "returns false without changing the stored selection" do
        expect(step.save!).to be(false)
        expect(store.at_school_period_gid).to be_nil
      end
    end
  end
  ...
  describe "validations" do
    context "when a school period has not been selected" do
      let(:at_school_period_gid) { nil }
      let(:selected_at_school_period) { nil }

      it "adds the correct error message" do
        expect(step).not_to be_valid
        expect(step.errors[:at_school_period_gid])
