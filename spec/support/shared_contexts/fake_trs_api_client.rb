...
shared_context 'fake trs api client that finds teacher that has failed their induction' do
  before do
...
shared_context 'fake trs api returns a teacher and then a teacher that has failed their induction' do
  before do
    allow(TRS::APIClient).to receive(:new).and_return(
      TRS::FakeAPIClient.new,
