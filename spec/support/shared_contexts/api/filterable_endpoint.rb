RSpec.shared_examples "a filter by cohort (registration period year) endpoint" do
  ...
  it "calls the correct query" do
    registration_period_years = "2023,2024"

    expect(query).to receive(:new).with(a_hash_including(
                                          lead_provider: active_lead_provider.lead_provider,
                                          registration_period_years:
                                        )).and_call_original

    authenticated_api_get(path, params: { filter: { cohort: registration_period_years } })
  end
...
RSpec.shared_examples "a filter by updated_since endpoint" do
  it "returns only resource that have been updated since the provided date" do
    resource_updated_one_week_ago = travel_to(1.week.ago) { create_resource(active_lead_provider:) }
    resource_updated_one_month_ago = travel_to(1.month.ago) { create_resource(active_lead_provider:) }

    # Resource updated more than two months ago should not be included.
    travel_to(1.year.ago) { create_resource(active_lead_provider:) }

    authenticated_api_get(path, params: { filter: { updated_since: 2.months.ago.iso8601 } })
  ...
  it "calls the correct query" do
    updated_since = 1.month.ago.iso8601

    expect(query).to receive(:new).with(a_hash_including(
                                          lead_provider: active_lead_provider.lead_provider,
                                          updated_since: Time.iso8601(updated_since)
                                        )).and_call_original

    authenticated_api_get(path, params: { filter: { updated_since: } })
  end

  it "decodes URL encoded dates" do
    updated_since = URI.encode_www_form_component(1.month.ago.iso8601)

    expect(query).to receive(:new).with(a_hash_including(
                                          lead_provider: active_lead_provider.lead_provider,
                                          updated_since: Time.iso8601(URI.decode_www_form_component(updated_since))
                                        )).and_call_original
