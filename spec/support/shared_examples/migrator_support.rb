RSpec.shared_examples "a migrator" do |model, dependencies|
  ...
  before do
    create_resource(migration_resource1)
    create_resource(migration_resource2)

    allow(described_class).to receive(:records_per_worker).and_return(records_per_worker)
    allow(FailureManager).to receive(:new).with(data_migration:) { failure_manager }
  end
  ...
  describe ".prepare!" do
    ...
    it "creates a pending data migrations" do
      expect { prepare! }.to change(DataMigration, :count).by(1)
      expect(DataMigration.all).to all(be_pending)
    end
    ...
    context "when there are enough models for two workers" do
      ...
      it "creates two pending data migrations" do
        expect { prepare! }.to change(DataMigration, :count).by(2)
        expect(DataMigration.all).to all(be_pending)
      end
    end
  end
  ...
  describe "#migrate!" do
    ...
    it "increments the processed count" do
      expect { migrate! }.to change { data_migration.reload.processed_count }.by(2).and(not_change { data_migration.failure_count })
    end

    it "sets the completed_at" do
      expect { migrate! }.to change { data_migration.reload.completed_at }.from(nil).to(be_within(5.seconds).of(Time.zone.now))
    end

    it "queues a follow up migration" do
      expect { migrate! }.to have_enqueued_job(MigrationJob)
    end
