RSpec.shared_context "it closes an induction" do
  ...
  let(:finished_on) { nil }
  let!(:ect_at_school_period) { FactoryBot.create(:ect_at_school_period, :with_training_period, teacher:, school:, started_on:, finished_on:) }
...
RSpec.shared_context "it closes an induction period and finishes any related periods" do
  ...
  let(:training_period) { ect_at_school_period.training_periods.first }
