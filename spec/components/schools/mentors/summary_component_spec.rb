RSpec.describe Schools::Mentors::SummaryComponent, type: :component do
  ...
  it "shows a message saying mentor is not registered for training" do
    ...
    expect(rendered_content).not_to have_css(".govuk-summary-list__row", text: "Lead provider")
    expect(rendered_content).not_to have_css(".govuk-summary-list__row", text: "Delivery partner")
  end

  context "when the mentor has training periods" do
    let!(:training_period) { FactoryBot.create(:training_period, :provider_led, :for_mentor, mentor_at_school_period:) }
    let(:lead_provider_name) { training_period.school_partnership.lead_provider.name }

    it "shows lead provider and delivery partner" do
      render_inline(described_class.new(mentor:, school:))
    ...
    context "when training period is withdrawn" do
      let!(:training_period) { FactoryBot.create(:training_period, :provider_led, :for_mentor, :withdrawn, mentor_at_school_period:) }
    ...
    context "when training period is deferred" do
      let!(:training_period) { FactoryBot.create(:training_period, :provider_led, :for_mentor, :deferred, mentor_at_school_period:) }
