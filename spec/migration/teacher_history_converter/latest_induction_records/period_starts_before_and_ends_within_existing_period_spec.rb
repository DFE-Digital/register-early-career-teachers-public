describe "Period starts before and ends within an existing period (scenario 1)" do
  subject(:actual_output) { ecf2_teacher_history.to_h }

  # Scenario 1
  #
  # When there are two induction records with different schools where the
  # second induction record starts before the first one and finishes within
  # the period described by the first one, we trim the end of the second one.
  #
  #           ┌──────────────────────┐
  #           │                      │
  #           └──────────────────────┘
  ...
  #      └────┘─────────────────┘
  #
  # This scenario might be resolved by the order of the induction records so may not occur?
  #
  ...
  let(:school_2) { Types::SchoolData.new(name: "School 2", urn: 789_012) }
  ...
  let(:expected_output) do
    {
      teacher: hash_including(
        ...
        ect_at_school_periods: [
          hash_including(
            started_on: Date.new(2023, 12, 1),
            finished_on: Date.new(2024, 1, 2),
            school:,
            training_periods: [
              hash_including(
                started_on: Date.new(2023, 12, 1),
