describe TeacherHistoryConverter::PremiumDeferralData do
  subject { described_class.new(state_changed_at:, states:, lead_provider_id:).deferral_data }
  ...
  context "when the are no 'deferred' states" do
    let(:states) { [active] }
  ...
  context "when there are 'deferred' states" do
    ...
    context "for a lead provider with 2 deferral states" do
      ...
      context "when the state_changed_at matches the day an event was created" do
        it "returns the matching deferral information" do
