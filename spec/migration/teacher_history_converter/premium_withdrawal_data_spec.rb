describe TeacherHistoryConverter::PremiumWithdrawalData do
  subject { described_class.new(state_changed_at:, states:, lead_provider_id:).withdrawal_data }
  ...
  let(:state_changed_at) { 5.years.ago.round }

  context "when the are no 'withdrawn' states" do
    let(:states) { [active] }
  ...
  context "when 'state_changed_at' is blank" do
    let(:lead_provider_id) { bpn_id }
    let(:state_changed_at) { nil }
  ...
  context "when there are 'withdrawn' states" do
    context "when the state_changed_at does not match any states" do
      let(:lead_provider_id) { bpn_id }
    ...
    context "for a lead provider with 2 withdrawal states" do
      let(:lead_provider_id) { bpn_id }
      let(:state_changed_at) { 1.year.ago.round }

      context "when the state_changed_at matches the day an event was created" do
        it "returns the matching withdrawal information" do
    ...
    context "for a lead provider with 1 withdrawal state and 1 non-withdrawal state" do
      ...
      let(:state_changed_at) { 3.years.ago.round }
