describe "Two induction records start on the same day, the longer one was created more recently" do
  ...
  it "produces two mentor at school periods with one training period each" do
  ...
  it "creates a 'stub' mentor at school period that lasts 1 day, immediately before the ongoing period" do
    ...
    aggregate_failures do
      expect(stub_mentor_at_school_period.started_on).to eql(Date.new(2024, 3, 1))
      expect(stub_mentor_at_school_period.finished_on).to eql(Date.new(2024, 3, 2))
    end
  end

  it "creates a finished mentor at school period that starts on the day the ongoing induction record started" do
    finished_mentor_at_school_period = subject.mentor_at_school_period_rows[1]

    aggregate_failures do
      expect(finished_mentor_at_school_period.started_on).to eql(Date.new(2024, 3, 3))
      expect(finished_mentor_at_school_period.finished_on).to eql(Date.new(2024, 9, 9))
    end
  end
