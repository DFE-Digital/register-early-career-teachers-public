describe "Earlier induction record starts before previous one finishes" do
  ...
  it "cuts the earlier mentor at school period off when the later one starts" do
    earliest_mentor_at_school_period = subject.mentor_at_school_period_rows.first

    expect(earliest_mentor_at_school_period.started_on).to eql(Date.new(2024, 2, 2))
    expect(earliest_mentor_at_school_period.finished_on).to eql(Date.new(2024, 4, 3))
  end
