describe "Mentor with a withdrawal that is carried forward in their other induction records" do
  ######
  #
  #  ┌────────┐  ┌───────────┐  ┌───────────┐  ┌───────────┐  ┌───────────┐
  #  │ Active │->| Withdrawn |->| Withdrawn |->| Withdrawn |->| Withdrawn |---->
  #  └────────┘  └───────────┘  └───────────┘  └───────────┘  └───────────┘
  #              †
  ...
  let!(:ecf1_induction_record_1) do
    FactoryBot.create(
      ...
      end_date: withdrawn_at,
      training_status: "active",
      schedule: ecf1_schedule
    )
  end

  let(:withdrawn_at) { user_created_at + 18.months }

  let!(:ecf1_withdrawn_state) do
    FactoryBot.create(
      :migration_participant_profile_state,
      :withdrawn,
      participant_profile: ecf1_participant_profile,
  ...
  let!(:ecf1_induction_record_3) do
    FactoryBot.create(
      ...
      participant_profile: ecf1_participant_profile,
      induction_programme: ecf1_induction_programme,
      created_at: withdrawn_at + 1.week,
      start_date: withdrawn_at + 1.week,
      end_date: withdrawn_at + 2.weeks,
      training_status: "withdrawn",
      schedule: ecf1_schedule
    )
  end
  ...
  let!(:ecf1_induction_record_4) do
    FactoryBot.create(
      ...
      participant_profile: ecf1_participant_profile,
      induction_programme: ecf1_induction_programme,
      created_at: withdrawn_at + 2.weeks,
      start_date: withdrawn_at + 2.weeks,
      end_date: withdrawn_at + 3.weeks,
      training_status: "withdrawn",
      schedule: ecf1_schedule
    )
  end
  ...
  let!(:ecf1_induction_record_5) do
    FactoryBot.create(
      ...
      participant_profile: ecf1_participant_profile,
      induction_programme: ecf1_induction_programme,
      created_at: withdrawn_at + 3.weeks,
      start_date: withdrawn_at + 3.weeks,
      end_date: nil,
      training_status: "withdrawn",
      schedule: ecf1_schedule
    )
  ...
  context "when in latest_induction_records mode (economy)" do
    ...
    it "sets the withdrawal time and reason on the training period" do
      training_period = teacher.mentor_at_school_periods.first.training_periods.first

      aggregate_failures do
        expect(training_period.withdrawn_at).to eql(ecf1_withdrawn_state.created_at)
        expect(training_period.withdrawal_reason).to eql(ecf1_withdrawn_state.reason.underscore)
      end
    end

    it "closes the withdrawn training_period" do
      training_period = teacher.mentor_at_school_periods.first.training_periods.first
      expected_closing_date = [training_period.started_on + 1.day, training_period.withdrawn_at.to_date].max

      expect(training_period.finished_on).to eq(expected_closing_date)
    end

    it "does not close the mentor_at_school_period" do
