describe Migrators::School do
  ...
  describe "#migrate!" do
    ...
    context "when the ECF school can't be found in RECT" do
      ...
      it "adds an error" do
        # DataMigration.create!(model: :school, worker: 0) && Migrators::School.new(worker: 0).migrate!
        expect(data_migration.reload.failure_count).to eq(1)
        expect(data_migration.migration_failures.count).to eq(1)
        expect(data_migration.migration_failures.first.failure_message).to eq("School #{ecf_school.urn} (#{ecf_school.name}) missing on RECT!")
      end
    end
    ...
    context "when fields mismatch" do
      ...
      it "adds mismatch errors" do
        ...
        expect(data_migration.migration_failures.order(:created_at).pluck(:failure_message))
          .to match_array(["School #{gias_school.urn} (#{gias_school.name}) mismatch value on field named 'address_line1': Address Line 1 on ECF whilst Another Address Line 1 expected on RECT!",
