describe ECF1TeacherHistory::ECT do
  ...
  describe "#induction_records" do
    let(:ect) { FactoryBot.build(:ecf1_teacher_history_ect, induction_records:) }

    before do
      allow(ect).to receive(:all_induction_records).and_return(induction_records)
    end

    {
      ...
      context "when there are #{expected_combinations} combinations in #{rows.count} records" do
        ...
        it "returns the right count" do
          expect(ect.induction_records(migration_mode: :latest_induction_records).count).to be(expected_combinations)
        end
      end
    end
  end
