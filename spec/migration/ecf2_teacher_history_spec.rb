describe ECF2TeacherHistory do
  ...
  describe "#save_all!" do
    ...
    describe "saving a teacher who is both ECT and Mentor" do
      let(:api_id) { SecureRandom.uuid }
      let(:api_ect_training_record_id) { SecureRandom.uuid }
      let(:api_mentor_training_record_id) { SecureRandom.uuid }
      let(:ect_pupil_premium_uplift) { true }
      let(:ect_sparsity_uplift) { false }
      let(:mentor_became_ineligible_for_funding_on) { 1.year.ago.to_date }
      let(:mentor_became_ineligible_for_funding_reason) { "completed_declaration_received" }
      ...
      let(:ect_training_period_row) do
        ECF2TeacherHistory::TrainingPeriodRow.new(
      ...
      let(:ect_at_school_period_row) do
        ECF2TeacherHistory::ECTAtSchoolPeriodRow.new(
          ...
          finished_on: 1.year.ago.to_date,
          school: school_a_data,
          email: "ect@example.org",
          mentorship_period_rows: [],
          training_period_rows: [ect_training_period_row]
        )
      end

      let(:mentor_training_period_row) do
        ECF2TeacherHistory::TrainingPeriodRow.new(
          started_on: 1.year.ago.to_date,
          finished_on: 1.month.ago.to_date,
          created_at:,
      ...
      let(:mentor_at_school_period_row) do
        ECF2TeacherHistory::MentorAtSchoolPeriodRow.new(
          started_on: 1.year.ago.to_date,
          finished_on: 1.month.ago.to_date,
          school: school_b_data,
          email: "mentor@example.org",
          training_period_rows: [mentor_training_period_row]
        )
      end

      let(:other_arguments) do
        {
          ect_at_school_period_rows: [ect_at_school_period_row],
          mentor_at_school_period_rows: [mentor_at_school_period_row]
        }
      end

      it "creates a single teacher record" do
        expect { subject.save_all! }.to change(Teacher, :count).by(1)
      end

      it "creates the teacher with all attributes" do
        ...
        aggregate_failures do
          ...
          expect(teacher.api_ect_training_record_id).to eql(api_ect_training_record_id)
          expect(teacher.api_mentor_training_record_id).to eql(api_mentor_training_record_id)
          expect(teacher.ect_pupil_premium_uplift).to be(true)
          expect(teacher.mentor_became_ineligible_for_funding_on).to eql(mentor_became_ineligible_for_funding_on)
        end
      end

      it "creates both ECT and mentor at school periods" do
        teacher = subject.save_all!
      ...
      it "creates the ECT at school period with correct data" do
        teacher = subject.save_all!
        ect_period = teacher.ect_at_school_periods.first

        aggregate_failures do
          expect(ect_period.started_on).to eql(2.years.ago.to_date)
          expect(ect_period.finished_on).to eql(1.year.ago.to_date)
          expect(ect_period.school).to eql(school_a)
          expect(ect_period.email).to eql("ect@example.org")
          expect(ect_period.training_periods.count).to be(1)
        end
      end

      it "creates the mentor at school period with correct data" do
        teacher = subject.save_all!
        mentor_period = teacher.mentor_at_school_periods.first

        aggregate_failures do
          expect(mentor_period.started_on).to eql(1.year.ago.to_date)
          expect(mentor_period.finished_on).to eql(1.month.ago.to_date)
          expect(mentor_period.school).to eql(school_b)
          expect(mentor_period.email).to eql("mentor@example.org")
          # NOTE: Mentors with school_led training don't have training periods
          # (only provider_led mentors do)
        end
      end
