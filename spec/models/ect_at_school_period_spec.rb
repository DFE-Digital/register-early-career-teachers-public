describe ECTAtSchoolPeriod do
  ...
  describe "#migrated_data_accurate?" do
    ...
    before do
      stub_const(
        "ECTAtSchoolPeriod::ACCURATE_TEACHER_HISTORIES_AFTER",
        Date.yesterday
      )
    end
    ...
    context "when the teacher has not been migrated" do
      ...
      context "when the record was created after `ACCURATE_TEACHER_HISTORIES_AFTER`" do
        let(:created_at) { Time.current }

        it { is_expected.to be_truthy }
      end

      context "when the record was created before `ACCURATE_TEACHER_HISTORIES_AFTER`" do
