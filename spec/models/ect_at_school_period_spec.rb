describe ECTAtSchoolPeriod do
  ...
  describe "associations" do
    ...
    describe "leaving/joining training periods" do
      ...
      describe "#display_training_period" do
        subject(:display_training_period) do
          ect_at_school_period.display_training_period
        end

        context "when a current_or_next_training_period exists" do
          it "returns the current_or_next_training_period" do
            expected_training_period =
              ect_at_school_period.current_or_next_training_period

            expect(display_training_period).to eql(expected_training_period)
          end

          it "prefers the current_or_next_training_period over the latest_training_period" do
            expect(display_training_period).to eql(most_recent_training_period)
          end
        end

        context "when no current_or_next_training_period exists" do
          before do
            oldest_training_period.update!(
              started_on: 23.months.ago.to_date,
              finished_on: 20.months.ago.to_date
            )

            most_recent_training_period.update!(
              started_on: 18.months.ago.to_date,
              finished_on: 12.months.ago.to_date
            )
          ...
          it "has no current_or_next_training_period" do
            current_or_next_training_period =
              ect_at_school_period.current_or_next_training_period

            expect(current_or_next_training_period).to be_nil
          end

          it "falls back to the latest_training_period" do
            expected_training_period =
              ect_at_school_period.latest_training_period
        ...
        context "when there are no training periods" do
          let!(:most_recent_training_period) { nil }
          let!(:oldest_training_period) { nil }

          it "returns nil" do
            expect(display_training_period).to be_nil
          end
        end
      end
    end
