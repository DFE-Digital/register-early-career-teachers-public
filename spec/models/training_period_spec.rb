    ...
    describe ".ect_training_periods_latest_first" do
      ...
      let!(:ect_training_period2) do
        FactoryBot.create(
          :training_period,
          :for_ect,
          started_on: (ect_at_school_period.started_on + 31.days),
          finished_on: ect_at_school_period.finished_on,
          ect_at_school_period:,
          school_partnership:
        )
      ...
      it "returns latest ect training period for lead_provider" do
        expect(TrainingPeriod.ect_training_periods_latest_first(teacher:, lead_provider:).first).to eq(ect_training_period2)
      end
    end
    ...
    describe ".mentor_training_periods_latest_first" do
      ...
      let!(:mentor_training_period1) do
        FactoryBot.create(
          ...
          :for_mentor,
          started_on: mentor_at_school_period.started_on,
          finished_on: (mentor_at_school_period.started_on + 30.days),
          mentor_at_school_period:,
          school_partnership:
        )
      end
      let!(:mentor_training_period2) do
        FactoryBot.create(
          :training_period,
          :for_mentor,
          started_on: (mentor_at_school_period.started_on + 31.days),
          finished_on: mentor_at_school_period.finished_on,
          mentor_at_school_period:,
          school_partnership:
        )
      ...
      it "returns latest mentor training period for lead_provider1" do
        expect(TrainingPeriod.mentor_training_periods_latest_first(teacher:, lead_provider:).first).to eq(mentor_training_period2)
      end
    end
  end
