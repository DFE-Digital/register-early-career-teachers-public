describe MentorAtSchoolPeriod do
  ...
  describe '#eligible_for_funding_and_first_provider_led_assignment?' do
    let(:school) { FactoryBot.create(:school) }
    let(:teacher) { FactoryBot.create(:teacher) }
    ...
    context 'when mentor is eligible for funding, ECT is provider-led, and the mentor has no ongoing training periods' do
      ...
      it 'returns true' do
        expect(mentor_at_school_period.eligible_for_funding_and_first_provider_led_assignment?(ect_at_school_period)).to be(true)
      end
    end
    ...
    context 'when mentor already has an ongoing training period' do
      ...
      it 'returns false' do
        expect(mentor_at_school_period.eligible_for_funding_and_first_provider_led_assignment?(ect_at_school_period)).to be(false)
      end
    end
