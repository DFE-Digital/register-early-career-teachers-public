describe AvailableProviderPairing do
  ...
  describe 'validation' do
    subject { FactoryBot.create(:available_provider_pairing) }
  ...
  describe 'scopes' do
    describe '.with_delivery_partner' do
      it 'selects records with a matching delivery partner id' do
        expect(described_class.with_delivery_partner(1001).to_sql).to end_with('WHERE "available_provider_pairings"."delivery_partner_id" = 1001')
      end
    end

    describe '.with_active_lead_provider' do
      it 'selects records with a matching active_lead_provider_id id' do
        expect(described_class.with_active_lead_provider(2002).to_sql).to end_with('WHERE "available_provider_pairings"."active_lead_provider_id" = 2002')
      end
    end
  end
end
