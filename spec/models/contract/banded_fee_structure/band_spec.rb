RSpec.describe Contract::BandedFeeStructure::Band, type: :model do
  ...
  describe "validations" do
    it { is_expected.to validate_presence_of(:priority).with_message("Priority is required") }
    it { is_expected.to validate_numericality_of(:priority).is_greater_than(0).only_integer.with_message("Priority must be a number greater than zero") }
  ...
  describe "#letter" do
    let(:banded_fee_structure) do
      FactoryBot.build_stubbed(
        ...
        :with_bands,
        declaration_capacities: [100, 100, 100, 100]
      )
    end
