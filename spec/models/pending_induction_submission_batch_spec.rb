RSpec.describe PendingInductionSubmissionBatch do
  ...
  describe "scopes" do
    describe ".for_appropriate_body_period" do
      ...
      let!(:batch3) { FactoryBot.create(:pending_induction_submission_batch, :claim, appropriate_body_period:) }

      it "returns batched submissions for the specified appropriate body" do
        expect(described_class.for_appropriate_body_period(appropriate_body_period)).to contain_exactly(batch1, batch2, batch3)
      end
    end
  end
  ...
  describe "class methods" do
    ...
    describe ".new_claim_for" do
      ...
      it "creates a batch claim" do
        ...
        expect(claim.appropriate_body_period).to eq(appropriate_body_period)
      end
    end

    describe ".new_action_for" do
      subject(:action) { described_class.new_action_for(appropriate_body_period:) }
      ...
      it "creates a batch action" do
        ...
        expect(action.appropriate_body_period).to eq(appropriate_body_period)
      end
    end
  end
  ...
  describe "#recorded_count" do
    it "stores number of submissions without errors" do
      ...
      expect(batch.tally).to eq({ uploaded_count: 1, processed_count: 5, errored_count: 1, released_count: 1, passed_count: 1, claimed_count: 1 })
    end
  end

  describe "metric tallying" do
    it "tallies row and submission metrics" do
      expect(batch.tally).to eq({ uploaded_count: 1, processed_count: 0, errored_count: 0, released_count: 0, passed_count: 0, claimed_count: 0 })
      ...
      expect(batch.tally).to eq({ uploaded_count: 1, processed_count: 15, errored_count: 1, released_count: 5, passed_count: 3, claimed_count: 2 })
      batch.update!(uploaded_count: 100, processed_count: 99, errored_count: 98, released_count: 97, passed_count: 95, claimed_count: 94)
      expect(batch.tally).to eq({ uploaded_count: 100, processed_count: 99, errored_count: 98, released_count: 97, passed_count: 95, claimed_count: 94 })
      batch.tally!
      expect(batch.tally).to eq({ uploaded_count: 1, processed_count: 15, errored_count: 1, released_count: 5, passed_count: 3, claimed_count: 2 })
