RSpec.describe Schools::Term do
  describe ".containing" do
    ...
    context "when the date is the first day of a term" do
      it "returns that term" do
        term = Schools::Term.containing(Date.new(2026, 6, 1))
    ...
    context "when the date is the last day of a term" do
      it "returns that term" do
        term = Schools::Term.containing(Date.new(2026, 5, 31))
  ...
  describe ".current" do
    it "covers today" do
      ...
      travel_to(today) do
        expect(Schools::Term.current.cover?(today)).to be true
      end
    end
  end

  describe "#cover?" do
    let(:term) { Schools::Term.new(Date.new(2026, 1, 1), Date.new(2026, 3, 31)) }
