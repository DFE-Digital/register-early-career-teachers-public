RSpec.describe "Lead provider guidance pages" do
...
private
  ...
  def given_i_am_on_the_api_guidance_page
    path = "/api/docs/lead-provider/guidance"
    page.goto(path)
  ...
  def when_i_click_guidance
    ...
    expect(page).to have_path("/api/docs/lead-provider/guidance/guidance-for-lead-providers")
  end

  def when_i_click_view_all_release_notes
    page.get_by_role("link", name: "View all release notes", exact: true).click
    expect(page).to have_path("/api/docs/lead-provider/guidance/release-notes")
  end
  ...
  def then_i_should_go_to_the_latest_release_note
    expect(page).to have_path("/api/docs/lead-provider/guidance/release-notes/#{latest_release_note.slug}")
  end
