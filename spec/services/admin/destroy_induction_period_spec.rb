RSpec.describe Admin::DestroyInductionPeriod do
  ...
  before do
    allow(Events::Record).to receive(:record_admin_deletes_induction_period!).and_return(true)
  end
  ...
  describe "#destroy_induction_period!" do
    ...
    it "records an event with the correct parameters" do
      expect(Events::Record).to receive(:record_admin_deletes_induction_period!).with(
        author:,
        teacher:,
        appropriate_body:,
