  ...
  describe "#govuk_header_arguments" do
    ...
    describe "navigation items" do
      ...
      context "when logged in as a dfe_staff_user" do
        ...
        let(:expected_product_team_visible_items) do
          {
            **expected_finance_visible_items,
            "Tools" => "/admin/tools"
          }
        end
        ...
        context "when the user is a product team member" do
          let(:current_user) { FactoryBot.build(:dfe_user, role: :product_team) }

          it { is_expected.to eql(expected_product_team_visible_items) }
        end
      end
      ...
      context "when looking at the API guidance" do
        let(:current_path) { "/api/docs/training/guidance" }
        let(:expected_visible_items) do
          {
            "Home" => "/api/docs/training/guidance",
            "Swagger API documentation" => "/api/docs/training/v3",
            "Release notes" => "/api/docs/training/guidance/release-notes",
            "Guidance" => "/api/docs/training/guidance/guidance-for-lead-providers"
          }
        end
          ...
          it "has the correct nodes and paths" do
            expect(subject).to eql(
              [
                ...
                { text: "Tools", href: "/admin/tools", active_when: "/admin/tools", if: :can_see_tools? }
              ]
            )
          end
        end
