RSpec.describe GIAS::Schools::SchoolPartnerships::Transfer do
  ...
  let(:period) { double("period", school: predecessor_school) }
  ...
  %i[mentor_at_school_period ect_at_school_period].each do |period_type|
    let(:object) do
      double(
        "object",
        school_partnership: predecessor_school_partnership,
        mentor_at_school_period: period_type == :mentor_at_school_period ? period : nil,
        ect_at_school_period: period_type == :ect_at_school_period ? period : nil
      )
    end
    ...
    context "when the period is already at the successor school" do
      let(:predecessor_school) { successor_school }
