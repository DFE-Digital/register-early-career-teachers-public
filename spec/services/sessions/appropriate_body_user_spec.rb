...
RSpec.describe Sessions::AppropriateBodyUser do
  ...
  subject(:appropriate_body_user) { described_class.new(email:, name:, dfe_sign_in_organisation_id:, last_active_at:) }

  it_behaves_like 'a session user' do
    let(:user_props) { { email:, name:, dfe_sign_in_organisation_id: } }
  end
