RSpec.describe Metadata::Manager do
  ...
  describe ".refresh_all_metadata!" do
    ...
    it "calls refresh_metadata! for each handler with async: true" do
      expect(Metadata::Handlers::School).to receive(:refresh_all_metadata!).with(async:, track_changes: false)
    ...
    context "when track_changes is true" do
      ...
      it "calls refresh_metadata! for each handler with async: false" do
        expect(Metadata::Handlers::School).to receive(:refresh_all_metadata!).with(async:, track_changes: true)
