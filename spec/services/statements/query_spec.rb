    ...
    describe "filtering" do
      describe "by lead provider" do
        ...
        it "does not filter by lead provider if none is supplied" do
          condition_string = %(" WHERE \"lead_providers\".\"id")
      ...
      describe "by registration_period" do
        let!(:registration_period1) { create(:registration_period) }
        let!(:registration_period2) { create(:registration_period) }
        let!(:registration_period3) { create(:registration_period) }

        context "when registration_period param omitted" do
          it "returns all statements" do
            statement1 = create(:statement, registration_period: registration_period1)
            statement2 = create(:statement, registration_period: registration_period2)
            statement3 = create(:statement, registration_period: registration_period3)

            expect(described_class.new.statements).to contain_exactly(statement1, statement2, statement3)
          end

          it "doesn't reference the registration period's year in the query" do
            column_name = %("registration_periods\".\"year")

            expect(described_class.new.scope.to_sql).not_to include(column_name)
            expect(described_class.new(registration_period_start_years: "2021").scope.to_sql).to include(column_name)
          end
        end

        it "filters by registration period" do
          _statement = create(:statement, registration_period: registration_period1)
          statement = create(:statement, registration_period: registration_period2)
          query = described_class.new(registration_period_start_years: registration_period2.year)

          expect(query.statements).to eq([statement])
        end

        it "filters by multiple registration periods" do
          statement1 = create(:statement, registration_period: registration_period1)
          statement2 = create(:statement, registration_period: registration_period2)
          statement3 = create(:statement, registration_period: registration_period3)
        ...
        it "returns no statements if no registration periods are found" do
          query = described_class.new(registration_period_start_years: "0000")

          expect(query.statements).to be_empty
        end

        it "does not filter by registration period if blank" do
          condition_string = %("year")
