RSpec.describe API::RequestMiddleware do
  ...
  describe "#call on a non-API path" do
    it "does not fire API::Request" do
      request.get "/"

      expect(API::Request).not_to have_received(:send_persist_api_request)
    end
  end

  describe "#call on an API path" do
    it "fires an API::Request" do
      ...
      expect(API::Request).to have_received(:send_persist_api_request).with(request_data, response_data, status, anything, mock_request_uuid)
    end
  end
  ...
  describe "#call on an API path when an exception happens in the job" do
    it "logs the exception and returns" do
      ...
      allow(API::Request).to receive(:send_persist_api_request).and_raise(StandardError)
