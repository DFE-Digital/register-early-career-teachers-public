...
We check the email address given and make sure it doesn't exist for an ongoing ECT or mentor record with a different TRN. This would mean any emails that are attached to an open ect_at_school period. This is because we shouldn't have email addresses that are the same for entirely different people, when both are are still undergoing training.
...
### Giving an ECT's appropriate body when the school in't reusing previous programme details
...
This will show all existing mentors that are still actively at the school - they have a open mentor_at_school period.
