...
RUN apk add --no-cache build-base linux-headers yaml-dev nodejs npm postgresql18-dev zlib-dev && \
...
# Gems that bin/dev needs but the app does not, e.g. foreman
ARG DEV_GEMS=''
RUN [ -z "${DEV_GEMS}" ] || gem install --no-document ${DEV_GEMS}
...
# Build runtime image
FROM ruby:4.0.6-alpine AS production
