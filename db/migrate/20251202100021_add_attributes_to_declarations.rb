class AddAttributesToDeclarations < ActiveRecord::Migration[8.0]
  def change
    create_enum :declaration_payment_statuses, %w[not_started eligible payable paid voided ineligible]
    create_enum :declaration_clawback_statuses, %w[not_started awaiting_clawback clawed_back]
    ...
    change_table :declarations, bulk: true do |t|
      ...
      t.references :payment_statement, null: true, foreign_key: { to_table: :statements }
      t.references :clawback_statement, null: true, foreign_key: { to_table: :statements }
