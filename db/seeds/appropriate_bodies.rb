...
angel_oak_dfe_sign_in_organisation_id = Rails.env.development? ? test_uuid : pre_prod_uuid

appropriate_bodies = [
  {
    body_type: "teaching_school_hub",
    name: "Angel Oak Academy", # Used by developers for DfE Sign In
    dfe_sign_in_organisation_id: angel_oak_dfe_sign_in_organisation_id,
  },
  {
    body_type: "national",
...
appropriate_bodies.each do |attrs|
  AppropriateBody.find_or_create_by!(name: attrs[:name]).tap do |appropriate_body|
    appropriate_body.update!(attrs)
