def create_declarations!(active_lead_provider, training_periods)
  ...
  print_seed_info(::Teachers::Name.new(teacher).full_name, indent: 4)
...
def log_declaration_info(declaration)
  print_seed_info("#{declaration.declaration_type} - #{declaration.overall_status} - #{declaration.declaration_date.to_date}", indent: 6)
end
...
ActiveLeadProvider.all.to_a.product(%i[ect mentor]).each do |active_lead_provider, teacher_type|
  print_seed_info("#{active_lead_provider.lead_provider.name} - #{teacher_type}", indent: 4)

  active_lead_provider.school_partnerships.each do |school_partnership|
