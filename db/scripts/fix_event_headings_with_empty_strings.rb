...
# Retrieve all the events with headings that "changed from `Something` to ''"
#
# Before: "Induction status changed from 'InProgress' to ''"
# After: "Induction status 'InProgress' removed"
Event.where("heading LIKE ?", "%to ''").find_each do |event|
  heading = event.heading.gsub("changed from ", "").gsub("to ''", "removed")
  event.update_column(:heading, heading)
end
