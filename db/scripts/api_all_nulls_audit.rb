...
# separately. One-off check for issue #3902. Run via:
#
...
# `[path, schema_violation?]` tuples for every JSON `nil` whose field is
# defined in the schema. `schema_violation?` is true when the schema does not
...
class NullFieldAuditor
  ...
  def nulls(api_json_response, schema_name)
    found = []
    audit(api_json_response, SWAGGER_SCHEMAS.fetch(schema_name), nil, found)
    found
  end

private

  def audit(api_json_response, schema, path, found)
    schema = resolve(schema)

    if api_json_response.nil?
      found << [path, !schema[:nullable]] if schema
      return
    end
    ...
    when Hash
      ...
      api_json_response.each do |key, child|
        ...
        audit(child, property, [path, key].compact.join("."), found)
      end
    when Array
      item_schema = schema[:items]
      api_json_response.each do |child|
        audit(child, item_schema, [path, "*"].compact.join("."), found)
      end
    end
  end
  ...
  api_endpoints.each do |api_endpoint|
    ...
    api_endpoint[:fetch].call(lead_provider).each do |scope, render_opts, serializer|
      ...
      scope.find_each(batch_size: 5_000) do |record|
        ...
        nulls = auditor.nulls(api_json_response, api_endpoint[:schema])
        next if nulls.empty?

        url = api_endpoint[:url].call(record, render_opts)
        nulls.each do |path, violation|
          paths_to_urls[path] << url
          violation_flags[path] = violation
        end
      end
    end

    findings[lead_provider.name][api_endpoint[:key]] = { paths_to_urls:, violation_flags: }
    next if paths_to_urls.empty?

    total = paths_to_urls.values.sum(&:size)
    violation_paths = violation_flags.count { |_, v| v }
    violation_total = paths_to_urls.sum { |path, urls| violation_flags[path] ? urls.size : 0 }
    suffix = violation_paths.zero? ? "" : " (#{violation_total} schema #{'violation'.pluralize(violation_total)} across #{violation_paths} #{'path'.pluralize(violation_paths)})"
    detail_lines << "  - #{api_endpoint[:key]}: #{total} nulls across #{paths_to_urls.size} #{'path'.pluralize(paths_to_urls.size)}#{suffix}"
  end

  lp_total = findings[lead_provider.name].values.sum { |entry| entry[:paths_to_urls].values.sum(&:size) }
  lp_violations = findings[lead_provider.name].values.sum do |entry|
    entry[:paths_to_urls].sum { |path, urls| entry[:violation_flags][path] ? urls.size : 0 }
  end
...
findings.each do |lead_provider_name, endpoint_findings|
  has_violations = endpoint_findings.any? do |_, entry|
    entry[:violation_flags].any? { |_, v| v }
  end
  next unless has_violations

  $stdout.puts "### #{lead_provider_name}"
  $stdout.puts
  endpoint_findings.each do |endpoint, entry|
    entry[:paths_to_urls].each do |path, urls|
      next unless entry[:violation_flags][path]
