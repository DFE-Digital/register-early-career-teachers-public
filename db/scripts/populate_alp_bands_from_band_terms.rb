# Remodelling the banded fee structure for framework agreements
# Migrate existing values from Contract::BandedFeeStructure::BandTerm to FrameworkAgreement::Band
#
FrameworkAgreement::Band.transaction do
  Contract::BandedFeeStructure::BandTerm
    .includes(banded_fee_structure: { contract: :framework_agreement })
    .find_each do |band_term|
      framework_agreement = band_term.banded_fee_structure.contract.framework_agreement
      allocation_order = band_term.banded_fee_structure.band_terms.sort_by(&:min_declarations).index(band_term) + 1
      alp_band = FrameworkAgreement::Band.find_or_initialize_by(framework_agreement:, allocation_order:)

      if alp_band.new_record? || band_term.capacity > alp_band.capacity
        alp_band.capacity = band_term.capacity
        alp_band.save!
      end

      band_term.update_column(:band_id, alp_band.id)
    end
end
