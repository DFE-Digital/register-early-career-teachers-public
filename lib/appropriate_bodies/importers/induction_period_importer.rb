      ...
      rows
        ...
        .each_with_object({}) do |(trn, rows), h|
          ...
          rows.each do |current|
            if current.legacy_appropriate_body_id.in?(old_abs) && current.finished_on.nil?
              # NOTE: this only affects one record
              if current.started_on >= cutoff_date
                current.finished_on = current.started_on + 1
                keep << current
                next
              end

              current.finished_on = cutoff_date
              keep << current
              next
            end
