...
| ------------ | ------------- | 
| `true` | Set for when ECTs and mentors who become eligible at any point. For example, when an ECT’s induction is recorded or a mentor is eligible to start training. A `true` status will remain once set. Other fields are used to indicate that training has stopped or to block participants from being registered including `induction_completion_date`, `mentor_funding_end_date`, or `reason`. | 
