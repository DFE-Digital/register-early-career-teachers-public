...
There are some custom API headers that can only be used in the sandbox, such as `X-With-Server-Date`, which allows for [testing the ability to submit declarations in the sandbox ahead of time](/api/docs/lead-provider/guidance-for-lead-providers/how-to-test-the-api-effectively#test-declaration-submissions-using-x-with-server-date).

Visit our [API testing guidance](/api/docs/lead-provider/guidance-for-lead-providers/how-to-test-the-api-effectively) for further information on testing the API.
