  ...
  class AttributesSerializer < Blueprinter::Base
    ...
    class << self
      ...
      def ecf_enrolment(training_period)
        ...
        earliest_trainee = training_period.for_ect? ? teacher.earliest_ect_at_school_period : teacher.earliest_mentor_at_school_period
        {
          training_record_id:,
          email: trainee.email,
          participant_type: training_period.for_ect? ? "ect" : "mentor",
          lead_provider: training_period.lead_provider.name,
          created_at: earliest_trainee&.created_at&.rfc3339,
        }
      end
    end
  end
