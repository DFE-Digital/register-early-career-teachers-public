module Migrators
  class Declaration < Migrators::Base
    ...
    SPECIAL_DECLARATIONS_PATH = "special_declarations.csv".freeze
    SPECIAL_DECLARATIONS = CSV.table(SPECIAL_DECLARATIONS_PATH).freeze
  ...
  private
    ...
    def build_training_period(teacher:, participant_declaration:, special_declaration:)
      ...
      contract_period = find_contract_period_by_year!(participant_declaration.cohort.year)
      delivery_partner = find_delivery_partner_by_api_id!(special_declaration[:delivery_partner_id])
      school = find_school_by_urn!(special_declaration[:urn])
      at_school_period = create_at_school_period(teacher:, school:, contract_period:, for_ect: participant_declaration.ect?)
      school_partnership = find_or_create_school_partnership(school:, lead_provider:, delivery_partner:, contract_period:)
      create_training_period(at_school_period:, school_partnership:)
    end

    def create_at_school_period(teacher:, school:, contract_period:, for_ect: true)
      started_on = Date.new(contract_period.year, 9, 1)
      finished_on = start_date + 1.day
      existing_at_school_periods = for_ect ? teacher.ect_at_school_periods : teacher.mentor_at_school_periods
      existing_at_school_periods.where(started_on: ..finished_on).order(started_on: :desc).each do |at_school_period|
