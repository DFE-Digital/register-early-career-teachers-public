class APISchoolPartnership
  ...
  attribute :school_partnership_api_id
  ...
  validates :delivery_partner_api_id, presence: { message: "Enter a '#/delivery_partner_id'." }, on: %i[update create]
  validate :school_partnership_exists, on: :update

  def create
    SchoolPartnerships::Create.new(blah).create!
  end

  def update
    SchoolPartnerships::Update.new(blah).update!
  end
