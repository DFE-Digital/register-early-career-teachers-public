class TrainingPeriod < ApplicationRecord
  ...
  validates :api_withdrawn_at, presence: true, if: -> { api_withdrawal_reason.present? }
  validates :api_withdrawal_reason, presence: true, if: -> { api_withdrawn_at.present? }
  validates :api_deferred_at, presence: true, if: -> { api_deferral_reason.present? }
...
private
  ...
  def withdrawn_deferred_are_mutually_exclusive
    return unless api_withdrawn_at.present? && api_deferred_at.present?
