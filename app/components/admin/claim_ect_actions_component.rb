...
module Admin
  class ClaimECTActionsComponent < ViewComponent::Base
    def initialize(teacher:, pending_induction_submission:, appropriate_body:)
      @teacher = teacher
    ...
    def registration_blocked?
      pending_induction_submission.exempt? || (teacher && induction&.current_induction_period)
    end
    ...
    def blocked_registration_message
      if pending_induction_submission.exempt?
        "You cannot import #{name}. Our records show that #{name} is exempt from completing their induction."
      else
        "You cannot import #{name}. Our records show that #{name} has an active induction period."
      end
    end
  ...
  private
    ...
    attr_reader :teacher, :pending_induction_submission, :appropriate_body, :induction
