  ...
  module Statements
    module PaymentOverview
      class IttecfEctpComponent < PaymentOverviewComponent
        def rows
          [
            ["ECTs output payment", { text: number_to_pounds(output_payment), numeric: true }],
            ["Mentors output payment", { text: number_to_pounds(mentors_output_payment), numeric: true }],
      ...
      private
        ...
        delegate :outputs, to: :flat_rate_calculator, prefix: :mentors, private: true

        def mentors_output_payment
          mentors_outputs.total_billable_amount
        end

        def mentors_clawbacks
          -mentors_outputs.total_refundable_amount
        end
      end
    end
  end
