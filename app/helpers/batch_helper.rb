module BatchHelper
  ...
  def batch_download_data_table(batch)
    ...
    govuk_table(
      caption: "Submissions with errors (#{error_submissions.count} rows)",
      head: ['TRN', 'Date of birth', 'Error messages'],
      rows: error_submissions.map do |submission|
        [
          submission.trn,
          submission.date_of_birth&.to_fs(:govuk) || '-',
          submission.error_messages.join(', ')
        ]
      end
    )
  end
