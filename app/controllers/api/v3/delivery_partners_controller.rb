module API
  module V3
    class DeliveryPartnersController < BaseController
      def index
        conditions = { contract_period_years:, sort: }
        paginated_delivery_partners = paginate(delivery_partners_query(conditions:).delivery_partners)
        delivery_partners = serializer.preload_query(paginated_delivery_partners)

        render json: to_json(delivery_partners)
      end

      def show
        render json: to_json(delivery_partners_query.delivery_partner_by_api_id(api_id))
      end

    private

      def delivery_partners_query(conditions: {})
        DeliveryPartners::Query.new(**(default_query_conditions.merge(conditions)).compact)
      end

      def default_query_conditions
        @default_query_conditions ||= {
          lead_provider_id: current_lead_provider.id,
        }
      end

      def serializer_options
        @serializer_options ||= {
          lead_provider_id: current_lead_provider.id
        }
      end

      def delivery_partner_params
        params.permit(:api_id, :sort)
      end

      def api_id
        delivery_partner_params[:api_id]
      end

      def sort
        delivery_partner_params[:sort]
      end

      def to_json(obj)
        serializer.render(obj, root: "data", **serializer_options)
      end

      def serializer
        DeliveryPartnerSerializer
      end
    end
  end
end
