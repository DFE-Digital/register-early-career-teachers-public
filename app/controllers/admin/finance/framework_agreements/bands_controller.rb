module Admin::Finance::FrameworkAgreements
  class BandsController < Admin::Finance::BaseController
    ...
    def create
      @band = FrameworkAgreements::Bands::Create.new(author: current_user, framework_agreement: @framework_agreement, capacity: band_params[:capacity]).create!
    ...
    def update
      FrameworkAgreements::Bands::Update.new(
        author: current_user,
        band: @band,
    ...
    def destroy
      ...
      FrameworkAgreements::Bands::Destroy.new(author: current_user, band: @band).destroy!

      redirect_to bands_path, notice: "#{label} deleted"
    rescue FrameworkAgreements::Bands::Destroy::DeletionError => e
  ...
  private
    ...
    def set_framework_agreement
      @framework_agreement = FrameworkAgreement
        ...
        .find(params.expect(:framework_agreement_id))
    end
    ...
    def redirect_unless_bands_can_be_added_and_removed
      unless @framework_agreement.bands_can_be_added_and_removed?
        redirect_to admin_contract_period_framework_agreement_bands_path(contract_period, @framework_agreement),
                    flash: {
    ...
    def band_path(band)
      admin_contract_period_framework_agreement_band_path(contract_period, @framework_agreement, band)
    end

    def bands_path
      admin_contract_period_framework_agreement_bands_path(contract_period, @framework_agreement)
    end
  end
end
