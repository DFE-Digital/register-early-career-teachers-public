# Event recording requires #event_author_params
#
class Events::AppropriateBodyBatchAuthor
  attr_reader :email, :name, :appropriate_body_period_id, :batch_id

  def initialize(email:, name:, appropriate_body_period_id:, batch_id:)
    @email = email
    @name = name
    @appropriate_body_period_id = appropriate_body_period_id
    @batch_id = batch_id
  end

  def event_author_params
    {
      author_email: email,
      author_name: name,
      appropriate_body_period_id:,
      author_type: :appropriate_body_user,
      pending_induction_submission_batch_id: batch_id,
    }
  end

  def dfe_user? = false
  def appropriate_body_user? = true
end
