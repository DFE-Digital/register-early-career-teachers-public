module Sessions
  module Users
    ...
    private
      ...
      delegate :appropriate_body_id, to: :user_info # currently an AppropriateBodyPeriod

      # @return [Sessions::Users::AppropriateBodyPersona]
      def appropriate_body_persona
        AppropriateBodyPersona.new(email:, name:, appropriate_body_period_id: appropriate_body_id)
      end
      ...
      # @return [Boolean]
      def appropriate_body_user?
        # Second phase criteria - TBC:
        # NOTE: DSI assoc will be removed from periods in future
        # TODO: Replace with an active AB period
        #
        # 1. dfe_sign_in_roles.include?('AppropriateBodyUser')
        # 2. AppropriateBodyPeriod
        #   .active
        #   .includes(:dfe_sign_in_organisation)
        #   .find_by(dfe_sign_in_organisation: { uuid: organisation.id })
        #   .present?
