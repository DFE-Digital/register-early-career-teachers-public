module Schools
  class Query
    ...
    attr_reader :scope, :registration_period_id

    def initialize(lead_provider: :ignore, urn: :ignore, updated_since: :ignore, registration_period_id: :ignore)
      @registration_period_id = registration_period_id
      @scope = eligible_schools.not_cip_only.select(
        ...
        "EXISTS(
          ...
          INNER JOIN registration_periods AS rp ON alp.registration_period_id = rp.year
            WHERE schools.id = s.id
            AND rp.year = #{registration_period_id}
            LIMIT 1
        ...
        "CASE
            ...
            WHEN EXISTS (
               SELECT 1 AS one
                 ...
                 WHERE schools.id = s.id
                 AND rp.year = #{registration_period_id}
                 AND alp.lead_provider_id = #{lead_provider.id}
                 LIMIT 1
            )
            THEN 'provider_led'
        ...
        END AS transient_expression_of_interest"
            ...
            .or(schools_with_expression_of_interests(lead_provider))
            .distinct
    ...
    def schools
      return School.none if ignore?(filter: registration_period_id)
  ...
  private
    ...
    def schools_with_existing_partnerships
      School.where(id: School.select("schools.id")
        .joins(school_partnerships: { lead_provider_delivery_partnership: { active_lead_provider: :registration_period } })
        .where(registration_periods: { year: registration_period_id }))
    end

    def schools_with_expression_of_interests(lead_provider)
      schools_with_ects_expressions_of_interest = School.select("schools.id")
        .joins(ect_at_school_periods: { training_periods: { expression_of_interest: %i[registration_period lead_provider] } })
      ...
      schools_with_mentors_expressions_of_interest = School.select("schools.id")
        .joins(mentor_at_school_periods: { training_periods: { expression_of_interest: %i[registration_period lead_provider] } })
        .where.not(training_periods: { expression_of_interest_id: nil })
        .where(registration_periods: { year: registration_period_id })
        .where(lead_provider: { id: lead_provider.id })
        .group("schools.id")
