  ...
  class Query
    ...
    def initialize(lead_provider: :ignore, urn: :ignore, updated_since: :ignore, contract_period_id: :ignore)
      ...
      @scope = eligible_schools.not_cip_only.select(
        ...
        transient_mentors_at_school(lead_provider.id),
        transient_ects_at_school_training_programme(lead_provider.id),
  ...
  private
    ...
    def transient_in_partnership
      "EXISTS(
        ...
        INNER JOIN contract_periods AS cp ON alp.contract_period_id = cp.year
        WHERE schools.id = s.id
        AND cp.year = #{contract_period_id}
        LIMIT 1
      ) AS transient_in_partnership"
    end

    def transient_mentors_at_school(lead_provider_id)
      "EXISTS(
        SELECT 1 AS one
        FROM schools AS s
    ...
    def transient_ects_at_school_training_programme(lead_provider_id)
      "(
