module APISeedData
  class ParityChecks < Base
    NUMBER_OF_RUNS = 10

    def plant
      return unless plantable?

      log_plant_info("parity_checks")

      log_seed_info("Endpoints:", colour: :blue)

      ParityCheck::SeedEndpoints.new.plant!
      ParityCheck::Endpoint.find_each(&method(:describe_endpoint))

      log_seed_info("Completed runs:", colour: :green, blank_lines_before: 1)
      ...
      NUMBER_OF_RUNS.times do
        ...
        rand(1..10).times do
          ...
          LeadProvider.find_each do |lead_provider|
            ...
            fail_request = fail_run && Faker::Boolean.boolean(true_ratio: 0.4)
  ...
  protected
    ...
    def plantable?
      super && ParityCheck::Run.none? && Rails.application.config.parity_check[:enabled]
    end
  ...
  private
    ...
    def describe_run(run)
      log_seed_info("#{run.mode.capitalize}, #{run.requests.size} requests, #{run.rect_performance_gain_ratio}x performance gain, #{run.match_rate}% match rate", indent: 4)
    end
    ...
    def create_in_progress_run
      modes = %i[concurrent sequential]
      mode = modes[ParityCheck::Run.count % modes.size]
